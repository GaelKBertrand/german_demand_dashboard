/* ============================================================================
   ...
   fill each with LIVE counts from data/<sector>.meta.json.
...
(function () {
  ...
  SECTORS.forEach(function (s) {
    ...
    // Fetch the small meta.json; fill or gracefully remove the stat line.
    fetch(metaUrl(s), { cache: "no-store" })
      .then(function (r) { if (!r.ok) throw new Error("no meta"); return r.json(); })
      .then(function (m) {
        var el = document.getElementById("stat-" + s.id);
        el.classList.remove("is-loading");
        el.innerHTML =
          '<span class="stat-num mono">' + fmt(m.total) + '</span>' +
          '<span class="stat-unit">postings</span>' +
          '<span class="stat-sep">·</span>' +
          '<span class="stat-range">' + (m.dateRange || "") + '</span>';
      })
      .catch(function () {
        var el = document.getElementById("stat-" + s.id);
        el.classList.remove("is-loading");
        el.classList.add("is-empty");
        el.innerHTML = '<span class="stat-pending">Add data/' + s.id + '.csv to see counts</span>';
      });
  });
})();
